    ...
    <main className="flex-1">
          ...
          <div className="rounded-2xl border border-white/10 bg-black/70 p-6">
            ...
            <div className="mt-6 space-y-3">
              {featuredDeals.slice(0, 4).map((deal) => (
                <div key={deal.id} className="market-card rounded-2xl p-4">
                  <div className="flex items-center justify-between">
                    <p className="font-semibold text-white">{deal.name}</p>
                    <span className="text-sm text-brand-gold">{deal.discount}</span>
                  </div>
      ...
      <section className="py-6">
        <div className="market-shell rounded-[2rem] bg-gradient-to-br from-black to-zinc-900 p-8">
          <h2 className="text-2xl font-semibold text-white">Global marketplace</h2>
          <p className="mt-3 max-w-2xl text-zinc-400">Shopping, restaurants, entertainment, travel, sport, and betting offers are now combined into a scalable, future-ready platform for users, partners, and admins.</p>
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {featuredDeals.slice(0, 6).map((deal) => (
              <div key={deal.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="font-semibold text-white">{deal.name}</p>
                <p className="mt-2 text-sm text-zinc-400">{deal.discount}</p>
                <Link href={`/deals/${deal.slug}`} className="mt-4 inline-flex text-sm text-brand-gold">View deal</Link>
              </div>
            ))}
          </div>
        </div>
